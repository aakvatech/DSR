	...
	for dip_read in self.dip_reading:
		...
		for total_row in doc.shift_fuel_item_totals:
			if fuel_item == total_row.fuel_item:
				set_total(total_row.name,total_row.doctype,'tank_usage_quantity',flt(total_row.tank_usage_quantity) + flt(dip_read.difference_quantity))
				item_available = True
		if item_available == False:
			add_total_row(fuel_item,doc.name,'tank_usage_quantity',dip_read.difference_quantity)
